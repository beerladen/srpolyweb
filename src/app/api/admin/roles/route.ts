...
import { createManagedAdminRole, getManagedAdminRoles } from "@/lib/admin-user-management";
...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, items: await getManagedAdminRoles() });
}
...
export async function POST(request: Request) {
  ...
  const result = await createManagedAdminRole(payload ?? {}, access.user);
